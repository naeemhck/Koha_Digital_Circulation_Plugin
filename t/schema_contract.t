...
like $source, qr/sub _stamp_schema_state \{.*?ON DUPLICATE KEY UPDATE.*?plugin_version\s*=\s*VALUES\(plugin_version\)/s,
    'schema version stamp is maintained by an idempotent upsert';
is scalar( () = $source =~ /CREATE TABLE IF NOT EXISTS/g ), 5, 'all tables are retry-safe';
like $source,
    qr/\$self->_migration_001\(\$dbh\);.*?\bbegin_work\b.*?\$self->_stamp_schema_state\(\$dbh\);.*?\$self->_verify_schema\(\$dbh\);.*?\bcommit\b/s,
    'migration is followed by transactional stamp maintenance and strict verification';
like $source, qr/Schema state must contain exactly one canonical row/,
    'verification requires one canonical schema-state row';
...
unlike $source, qr/\b(?:AddIssue|AddReturn)\b/, 'upgrade source does not mutate native circulation';
done_testing;

...
Assert-Contract ($source.Contains("our `$VERSION             = '0.2.1';")) 'plugin version is 0.2.1'
...
Assert-Contract ($staffJs.Contains('#circ-menu ul,nav[aria-label="Circulation"] ul') -and $staffJs.Contains('dataset.jzlUrl')) 'Circulation shortcut targets Circulation menu selectors with data-jzl-url'
...
if ($Kpz) {
    ...
    Assert-Contract (
        (Split-Path -Leaf $resolvedKpz) -match '^JunaidZaidiLibrary-DigitalCirculation-v0\.2\.1(?:-[0-9A-Za-z][0-9A-Za-z.-]*)?\.kpz$'
    ...
    Assert-Contract ($packagedMain.Contains("our `$VERSION             = '0.2.1';")) 'packaged internal plugin version is 0.2.1'

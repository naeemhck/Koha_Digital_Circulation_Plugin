...
3. Run `./scripts/validate-package.sh dist/JunaidZaidiLibrary-DigitalCirculation-v0.2.2.kpz` and `unzip -l` to inspect every member. Confirm the package includes the Circulation shortcut staff asset and does **not** include an authoritative return endpoint.
...
5. Run the plugin installation/upgrade action and confirm schema version remains **1** and plugin version is **0.2.2**. DDL is idempotent, serialized with a named database lock, and each migration is recorded. Patch upgrades only refresh the recorded `plugin_version` stamp; existing tables/data are never dropped.

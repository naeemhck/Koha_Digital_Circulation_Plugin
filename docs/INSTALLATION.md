...
3. Run `./scripts/validate-package.sh dist/JunaidZaidiLibrary-DigitalCirculation-v0.3.0.kpz` and `unzip -l` to inspect every member. Confirm the package includes renewal, revocation, expiry, return, and Circulation-shortcut runtime.
...
5. Run the plugin installation/upgrade action and confirm schema version remains **1** and plugin version is **0.3.0**. DDL is idempotent, serialized with a named database lock, and each migration is recorded. Upgrades refresh only the recorded `plugin_version` stamp; existing tables/data are never dropped.

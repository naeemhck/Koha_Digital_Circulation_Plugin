...
3. Run `./scripts/validate-package.sh dist/JunaidZaidiLibrary-DigitalCirculation-v0.4.0.kpz` and `unzip -l` to inspect every member. Confirm the package includes lifecycle, Circulation-shortcut, and Saved Reports runtime.
...
5. Run the plugin installation/upgrade action and confirm schema version remains **1** and plugin version is **0.4.0**. DDL is idempotent, serialized with a named database lock, and each migration is recorded. Upgrades refresh only the recorded `plugin_version` stamp; existing tables/data are never dropped.
6. Confirm the native Saved Reports group **Digital Circulation / eBooks**
   contains ten non-public managed definitions. If the institution uses the
   `EBOOK` item type, select it at runtime; item types are catalogue metadata
   and are not provisioned by this plugin.
